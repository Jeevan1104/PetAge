...
export default function LoginPage() {
  ...
  const { firebaseUser, loading, error, signIn, signInWithGoogle, clearError } = useAuthStore();
    ...
    <div className="min-h-screen bg-surface flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-[400px]">
          ...
          <p className="text-body-sm text-text-secondary mb-8">Access your pet health records</p>
          ...
          <div className="text-center mt-6 space-y-2">
            <Link href="/forgot-password" className="text-body-sm text-clinical-blue font-medium hover:underline block">
              Forgot your password?
            </Link>
            <p className="text-body-sm text-text-secondary">
              Don&apos;t have an account?{" "}
              <Link href="/signup" className="text-clinical-blue font-medium hover:underline">Sign up</Link>
            </p>
          </div>
        </div>
      </div>
