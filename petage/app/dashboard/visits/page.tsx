...
function VisitsContent() {
  ...
  const { visits, loading, error, fetchVisits } = useVisitStore();
    ...
    <div className="min-h-screen bg-surface pb-24 animate-fade-in relative">
      ...
      <header className="px-6 pt-6 pb-4">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-h1 text-navy">Vet Visits</h1>
        </div>
      </header>
      ...
      <main className="px-6">
        {error ? (
          ...
          <VisitList visits={visits} petId={petId} />
        )}
      </main>

      {!!petId && (
        <FAB 
          onClick={() => router.push(`/dashboard/visits/new?petId=${petId}`)} 
        />
      )}
    </div>
  );
